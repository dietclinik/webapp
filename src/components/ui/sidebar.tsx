...
>(
  (
    ...
    const handleMouseLeave = () => {
        if (collapsible === 'icon' && state === 'expanded' && open) {
            setOpen(false);
        }
    };
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-7 w-auto px-2", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex flex-col gap-2 p-4 group-data-[collapsible=icon]/sidebar:p-2 group-data-[collapsible=icon]/sidebar:items-center group-data-[collapsible=icon]/sidebar:justify-center", className)}
      {...props}
    />
  )
})
    ...
    const buttonChildren = React.Children.map(children, (child) => {
      if (React.isValidElement(child) && child.type === 'svg') {
        return React.cloneElement(child as React.ReactElement<any>, {
          style: {
            width: 'var(--sidebar-icon-size, 24px)',
            height: 'var(--sidebar-icon-size, 24px)',
          },
      ...
      if (React.isValidElement(child) && child.type === 'span') {
         return React.cloneElement(child as React.ReactElement<any>, {
            style: {
                fontSize: 'var(--sidebar-font-size, 16px)',
            },
        });
      }
      return child;
