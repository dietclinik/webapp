...
'use server';
/**
 * @fileOverview A flow to handle the creation of a new customer by a partner.
 *
 * - sendPartnerCustomerWelcomeEmail - Handles user creation, data saving, and email notifications.
 */
...
const PartnerCustomerWelcomeInputSchema = z.object({
    partnerId: z.string(),
    ...
    customerData: z.object({
        ...
        moneyBackChallenge: z.boolean().optional(),
...
export type PartnerCustomerWelcomeInput = z.infer<typeof PartnerCustomerWelcomeInputSchema>;

export async function sendPartnerCustomerWelcomeEmail(input: PartnerCustomerWelcomeInput): Promise<{ userId?: string; message: string }> {
  return newPartnerCustomerFlow(input);
}
...
const sendEmailFlow = ai.defineFlow({
    name: 'sendPartnerCustomerEmailFlow',
    ...
    try {
        ...
        console.log(`Partner customer welcome email sent to ${email}`);
    } catch (error) {
        console.error("Error sending partner customer email:", error);
    }
});

const newPartnerCustomerFlow = ai.defineFlow(
  {
    name: 'newPartnerCustomerFlow',
    inputSchema: PartnerCustomerWelcomeInputSchema,
    outputSchema: z.object({ userId: z.string().optional(), message: z.string() }),
  },
  async ({ partnerId, partnerName, customerData, profileData }) => {
    ...
    const dataToSave = {
        ...
        status: 'Active',
        paymentStatus: 'Paid', // Partner-added customers are considered paid
        since: format(startDate, "yyyy-MM-dd"),
        isNew: true,
        dietPlanId: null,
        subscriptionStartDate: Timestamp.fromDate(startDate),
        subscriptionEndDate: Timestamp.fromDate(endDate),
        moneyBackChallenge: customerData.moneyBackChallenge || false,
        challengeDuration: customerData.challengeDuration || null,
    };
    ...
    await createNotification({
        userId: 'admin',
        message: `${name} has been added as a customer by partner ${partnerName}.`,
        link: `/admin/customers/view/${userId}`
    });
     
    await createNotification({
        userId: partnerId,
        message: `You have successfully added ${name} as a new customer.`,
