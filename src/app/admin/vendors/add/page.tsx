  ...
  const onSubmit = async (data: VendorFormData) => {
    ...
    try {
      const result = await processVendor({
        paymentSuccess: true, // Admin-added vendors are considered successful payments
        vendorData: {
            name: data.name,
            email: data.email,
            mobile: data.mobile,
      ...
      toast({ variant: "destructive", title: "Error", description: `Could not create vendor: ${error.message}` });
    } finally {
      ...
      <CardHeader>
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <CardDescription>Fill in the details to add a new vendor vendor.</CardDescription>
          </div>
        </div>
      </CardHeader>
      <Form {...form}>
          ...
          <CardContent className="space-y-6 max-w-2xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField control={form.control} name="name" render={({ field }) => (
                <FormItem><FormLabel>Vendor Name *</FormLabel><FormControl><Input placeholder="Fitness World" {...field} /></FormControl><FormMessage /></FormItem>
              )} />
