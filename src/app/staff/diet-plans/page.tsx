...
export default function DietPlansPage() {
  ...
  const fetchPlansAndUsers = async () => {
    ...
    try {
      // Fetch diet plans created by the logged-in staff member
      const dietPlansRef = collection(db, "dietPlans");
      const qPlans = query(dietPlansRef, where("createdBy", "==", staffId));
      
      const planSnapshots = await getDocs(qPlans);
      ...
      setDietPlans(fetchedPlans);
        ...
        if (error.code === 'permission-denied') {
             errorEmitter.emit('permission-error', new FirestorePermissionError({ path: "dietPlans", operation: 'list' }));
        } else {
  ...
  const fetchCustomers = async () => {
    ...
    const qCustomers = query(customersCollectionRef, where("assignedStaffId", "==", auth.currentUser.uid));
    try {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
                <Link href="/staff/diet-plans/builder">
                ...
                <TableCell>
                  <TooltipProvider>
                    <div className="flex items-center justify-center gap-2">
                         ...
                         <Tooltip>
                            <TooltipTrigger asChild>
                                <Link href={`/staff/diet-plans/builder?planId=${plan.id}`}>
