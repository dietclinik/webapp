...
export default function StaffProfilePage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        setUserId(user.uid);
        const profileDocRef = doc(db, "staff", user.uid);
        try {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
        if(imageFile && storage) {
            const storageRef = ref(storage, `staff_pictures/${userId}/${imageFile.name}`);
        ...
        const profileDocRef = doc(db, "staff", userId);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Update your personal and contact information.</p>
      </div>
              ...
              <CardContent>
                <form className="grid gap-6 md:grid-cols-2" onSubmit={(e) => { e.preventDefault(); handleSaveChanges(); }}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="experience" type="number" value={profile.experience} onChange={(e) => setProfile(p => ({...p, experience: Number(e.target.value)}))} />
                  </div>
