...
import { Home, Settings, ShieldAlert, Users, Video, IndianRupee, CreditCard } from 'lucide-react';
            ...
            <Card className="w-full max-w-md mx-4">
                ...
                <CardContent className="text-center">
                    <p className="text-muted-foreground">
                        You do not have permission to view the vendor portal. Please contact your administrator.
                    </p>
                    <Button asChild className="mt-6">
                        <Link href="/vendor/login">Return to Login</Link>
                    </Button>
                </CardContent>
            </Card>
        </div>
...
function VendorAuthWrapper({ children }: { children: ReactNode }) {
  ...
  const { toast } = useToast();
            ...
            if (vendorDocSnap.exists()) {
                ...
                router.push('/vendor/login');
                 toast({
                    variant: "destructive",
                    title: "Access Denied",
                    description: "You do not have permission to access the vendor portal.",
                });
            }
        } catch (error) {
    ...
    const navLinks = [
        ...
        { href: "/vendor/customers", icon: Users, label: "My Customers", activePaths: ["/vendor/customers/add", "/vendor/customers/edit"] },
        { href: "/vendor/earnings", icon: IndianRupee, label: "Earnings" },
        { href: "/vendor/meetings", icon: Video, label: "My Meetings" },
        { href: "/vendor/subscription", icon: CreditCard, label: "Subscription" },
        { href: "/vendor/profile", icon: Settings, label: "My Profile" },
    ];
...
export default function VendorLayout({ children }: { children: ReactNode }) {
  ...
  if (pathname === '/vendor/login' || pathname.startsWith('/vendor/meetings/join')) {
    return <>{children}</>;
  }

  return <VendorAuthWrapper>{children}</VendorAuthWrapper>;
}
