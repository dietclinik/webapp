  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (vendorDocSnap.exists()) {
            ...
            await auth.signOut();
            toast({ variant: "destructive", title: "Access Denied", description: "You are not an authorized vendor." });
        }
    } catch(error: any) {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
        const res = await fetch('/api/auth/whatsapp/send-otp', {
            ...
            body: JSON.stringify({ phoneNumber: fullNumber, userType: 'vendor' })
        });
       ...
       <main className="flex-1 flex items-center justify-center">
        <div className="flex flex-col items-center space-y-4">
            ...
            <CardHeader className="text-center">
                ...
                <CardDescription>Access the vendor portal.</CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <TabsContent value="email">
                        <form onSubmit={handleLogin} className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <Input id="vendor-email" type="email" placeholder="owner@example.com" required value={email} onChange={(e) => setEmail(e.target.value)} />
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="vendor-password">Password</Label>
                                <div className="relative">
                                    <Input id="vendor-password" type={showPassword ? "text" : "password"} required value={password} onChange={(e) => setPassword(e.target.value)} className="pr-10" />
