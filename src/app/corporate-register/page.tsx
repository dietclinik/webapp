...
type Plan = {
  ...
  tag?: string;
  displayOrder?: number;
  planFor?: 'customer' | 'vendor' | 'corporate';
  maxCustomers?: number;
  features?: string[];
...
export default function CorporatePlansPage() {
  ...
  useEffect(() => {
    ...
    const fetchPlans = async () => {
        ...
        try {
            ...
            const q = query(plansCollectionRef, where("status", "==", "Active"), where("showOnFrontend", "==", true), where("planFor", "==", "corporate"));
            ...
            console.error("Error fetching corporate subscription plans:", error);
        } finally {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      <Header variant="dark" showNavLinks={false} loginUrl="/corporate/login" />
      <main className="flex-1">
        <section className="mx-auto py-12 md:py-24 lg:py-32 bg-muted/50">
          <div className="container px-4 md:px-6">
            <div className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
                <h1 className="text-3xl font-bold tracking-tighter sm:text-5xl font-headline">Corporate Subscription Plans</h1>
                <p className="max-w-[900px] text-muted-foreground md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed">
                    Choose a plan to provide health benefits to your employees.
                </p>
            </div>
            {loading ? (
                ...
                {plans.map((plan) => (
                        ...
                        <div className="text-center mb-4 space-y-2">
                            ...
                            {plan.maxCustomers !== undefined && (
                                <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
                                    ...
                                    <span>Up to {plan.maxCustomers} Employees</span>
                                </div>
                            )}
                        </div>
                    ...
                    <CardFooter>
                      <Link href={`/corporate-register/${plan.id}`} className="w-full">
                ...
                <div className="text-center text-muted-foreground">
                    <p>Corporate pricing plans are not available at the moment. Please check back later.</p>
                </div>
            )}
          </div>
        </section>
